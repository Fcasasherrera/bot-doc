...
@section('scripts')
    ...
    <script>
        ...
        function createCite() {
            ...
            let stringS = '';
            sintomas.map((sin, index) => {
                if ((index + 1) === sintomas.length) {
                    stringS = stringS + sin;
                } else {
                    stringS = stringS + sin + ':';
                }
            ...
            if (pruebas.length > 0) {
                let stringP = '';
                pruebas.map((sin, index) => {
                    if ((index + 1) === pruebas.length) {
                        stringP = stringP + sin;
                    } else {
                        stringP = stringP + sin + ':';
                    }
                })
                data.pruebas_lab = `${stringP}`;
            }
            if (signos.length > 0) {
                let stringSi = '';
                signos.map((sin, index) => {
                    if ((index + 1) === signos.length) {
                        stringSi = stringSi + sin;
                    } else {
                        stringSi = stringSi + sin + ':';
                    }
                })
                data.signos = `${stringSi}`;
            }
            console.log(data)
